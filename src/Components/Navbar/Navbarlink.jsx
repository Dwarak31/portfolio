      ...
      {links.map((link, index) => {
        return (
          <li key={index} className="group">
            <Link
              to={link.section} smooth={true} spy={true} duration={500} offset={-130}
              className="cursor-pointer text-white hover:text-cyan transition-all duration-300 "
            >
              {link.link}
            </Link>
            <div className="mx-auto by-cyan w-0 group-hover:w-full h-[1px] transition-all duration-300"></div>
          </li>
        );
      })}
    </ul>
